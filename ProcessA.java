...
class SenderA implements Runnable{
    ...
    public void run() {
        while (true) {
            try {
                ...
                byte ttl = 8;
                byte seqNo = 0;
                byte message[] = {RED, A, 8, 0, 'H', 'i', ' ','f', 'r', 'o', 'm', ' ', 'A'};
                InetAddress ip = InetAddress.getLocalHost();
                dp = new DatagramPacket(message, message.length, ip, 4002);
                ds.send(dp);
                Thread.sleep(5000);
                }
            catch (Exception e) {
...
class ReceiverA implements Runnable{
            ...
            while (true) {
                ...
                if(color == RED){
                    ...
                    if(receive[2]>0) {
                        ...
                        InetAddress ip = InetAddress.getLocalHost();
                        receive[0] = GREEN;
                        dp = new DatagramPacket(receive, i, ip, 4002);
                        ds.send(dp);
                    ...
                    if(receive[2]>0){
                        ds = new DatagramSocket();
                        InetAddress ip = InetAddress.getLocalHost();
                        DatagramPacket dp = new DatagramPacket(receive, i, ip, 4002);
                        ds.send(dp);
                        //Thread.sleep(5000);
                    }
                    else if (receive[2] == 0){
...
public class ProcessA {
    ...
    public static void main(String args[]) {
        ...
        Thread sender = new Thread(new SenderA());
        Thread receiver = new Thread(new ReceiverA());
        sender.start();
        receiver.start();
    }
}
