...
class SenderC implements Runnable{
    ...
    public void run() {
        try {
            while (true) {
                ...
                byte ttl = 8;
                byte seqNo = 0;
                byte message[] = {RED, C, 8, 0, 'H', 'i',' ','f','r','o','m',' ','C'};
                InetAddress ip = InetAddress.getLocalHost();
                dp = new DatagramPacket(message, message.length, ip, 4001);
                ds.send(dp);
                Thread.sleep(5000);
            }
        }
            ...
            while (true) {
                ...
                if(color == RED){
                    ...
                    if(receive[2]>0){
                        ds = new DatagramSocket();
                        InetAddress ip = InetAddress.getLocalHost();
                        DatagramPacket dp = new DatagramPacket(receive, i, ip, 4001);
                        ds.send(dp);
                        System.out.println("Green Says Hello");
                        //Thread.sleep(5000);
                    }
                    else if (receive[2] == 0){
                        receive[0] = BLUE;
...
public class ProcessC {
    ...
    public static void main(String args[]) {
        ...
        Thread sender = new Thread(new SenderC());
        Thread receiver = new Thread(new ReceiverC());
        sender.start();
        receiver.start();
    }
}
